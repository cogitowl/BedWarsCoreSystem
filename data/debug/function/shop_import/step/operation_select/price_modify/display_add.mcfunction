...
# 更改数据
$execute as @e[type=marker,distance=..0.5,tag=price_modify,limit=1] store result score @s temp run data get storage bw:shop_display/$(ShopId)/pg$(Page) data[{Slot:$(Slot)b}].Lore.Price.$(Currency)
...
$execute store result storage bw:shop_display/$(ShopId)/pg$(Page) data[{Slot:$(Slot)b}].Lore.Price.$(Currency) short 1 run scoreboard players get @e[type=marker,distance=..0.5,tag=price_modify,limit=1] temp
